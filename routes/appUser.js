...
* 获取随机图片验证码
* */
router.get('/web/currency/api/captcha', (req, res, next)=>{
    // 1. 生成随机的验证码
...
* 用户名和密码登录
* */
router.post('/web/currency/api/login_pwd', (req, res)=>{
    console.log('----', req.body);
    //  1. 获取数据
    let user_name = req.body.user_name;
    let user_pwd = md5(req.body.user_pwd);
    let captcha = req.body.captcha.toLowerCase();
    console.log('/web/currency/api/login_pwd', user_name, user_pwd, captcha, req.session);
 ...
 * 获取短信验证码
 */
router.get('/web/currency/api/send_code', (req, res)=>{
    // 1. 获取手机号
...
* 手机验证码登录
* */
router.post('/web/currency/api/login_code', (req, res, next)=>{
    // 1. 获取数据
...
router.get('/web/currency/api/userinfo', (req, res, next)=>{
    // 1. 取出userId
...
* 退出登录
* */
router.get('/web/currency/api/login', (req, res)=>{
    // 清除session中的userid
    delete req.session.userid;
    // 返回数据
